...
export default function Dust({ concentration }: DustProps) {
  const getDustInfo = (concentration: number) => {
    if (concentration >= 152) { return { grade: '매우나쁨', color: Colors.red, }; }
    else if (concentration >= 110) { return { grade: '나쁨', color: '#A47A95', }; }
    else if (concentration >= 80) { return { grade: '보통', color: '#7A7FA4', }; }
    else if (concentration >= 25) { return { grade: '좋음', color: '#7AA4A1', }; }
    else { return { grade: '매우좋음', color: '#2196F3', }; }
  };
