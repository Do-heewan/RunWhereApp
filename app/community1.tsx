...
},
{
  ...
  time: '이번 주 금요일 18:00',
  location: '양재천 산책로',
  description: '러닝 후 간단한 스트레칭으로 몸과 마음 힐링해요!',
  hashtags: ['#힐링런', '#스트레칭', '#양재천', '#금요러닝'],
  participants: 10,
  maxParticipants: 10,
  organizer: {
    name: '힐링메이트',
      ...
      <View style={styles.header}>
        ...
        {TABS.map(tab => {
          ...
          return (
            ...
            >
              {focused ? (
                <GradientText text={tab} style={styles.tabTxt} />
              ) : (
                <Text style={[styles.tabTxt, { color: '#888' }]}>{tab}</Text>
              )}
            </TouchableOpacity>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  tabTxt: { color: '#8E8E93', fontSize: 16, fontWeight: 'bold', textAlign: 'center',},
